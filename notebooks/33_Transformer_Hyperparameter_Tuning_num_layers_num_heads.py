...
def create_model_hyperparameter(hp): 
  ...
  model = Transformer(
    num_layers=hp.Choice('num_layers', values=[2,4,6,8,10]),
    d_model=d_model,
    num_heads=hp.Choice('num_heads', values=[2,4,6,8,10]),
    dff=dff,
...
tuner = GridSearch(
    ...
    project_name = 'hyperparameter_tuning_transformer_layers_heads'
)
