...
def create_model_hyperparameter(hp): 
  ...
  model = Transformer(
    ...
    d_model=hp.Choice('d_model', values=[16,32,64,96,128,256,512]),
    num_heads=num_heads,
    dff=dff,
...
tuner = GridSearch(
    ...
    project_name = 'hyperparameter_tuning_transformer_embedding_dimension'
)

# Early stopping
stop_early = tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=5, min_delta = 0.001)
